...
_9.1.2. Агрегатные функции языка_
...
_Рис. 1.2. Пример ER-диаграммы базы данных ФАКУЛЬТЕТ_
...
![Рис. 2.1. Трехуровневая архитектура СУБД](/2.1.jpg)
...
![Рис. 2.2. Структура системы БД с выделением клиентов и сервера](/2.2.jpg)
...
_Рис. 2.4. Модель удаленного доступа_
...
_Рис. 2.6. Архитектура трехуровневой модели_
...
![Рис. 3.1. Жизненный цикл БД](/3.1.jpg)
...
![Рис. 3.2. Обозначения элементов диаграммы](/3.2.jpg)
...
![Рис. 3.4. Диаграмма представлений сущности ТОВАР и ее атрибутов](/3.4.jpg)
...
![Рис. 3.6. Диаграмма типа связи ЧИТАЕТ](/3.6.jpg)
...
![Рис. 3.9. Диаграмма с использованием понятий супертип и подтип](/3.9.jpg)
...
![Рис. 3.10. Форма поставки](/3.10.jpg)
...
![Рис. 3.11. ПрО ПОСТАВКА](/3.11.jpg)
...
![Рис. 3.12. Диаграмма модели предметной области ПОСТАВКА](/3.12.jpg)
...
![Рис. 4.3. Запись Сотрудник](/4.3.jpg)
...
_Рис. 4.6. Набор Работает между двумя типами записей Отдел и Сотрудник_
...
_Рис. 4.7. Иерархическая модель данных_
...
следующим образом: **{Таб\_ном\_преп, Фамилия, Должность}**  
Или
...
|Таб\_ном\_преп| Фамилия |Должность|
|--------------------|-------------|-------|

Отношение строится с учетом ряда факторов. Каждому имени атрибута Аi, 1 >= i <= n ставится в соответствие множество допустимых для соответствующего столбца значений. Это множество Di, называется доменом данного имени атрибута.
...
Эти отображения называются _кортежами_. Каждый кортеж
отношения отображает экземпляр сущности, а атрибут отношения
отображает атрибут сущности.

Множество кортежей называется _телом отношения_. Тело отношения
отражает состояние сущности, поэтому во времени оно постоянно
меняется. Тело отношения характеризуется _кардинальным числом_, которое
равно количеству содержащихся в нем кортежей.

Одной из главных характеристик отношения является его степень.
_Степень отношения_ определяется количеством атрибутов, которое в нем
...
![Рис. 5.1. Отношение СОТРУДНИК](/5.1.jpg)

_Рис. 5.1. Отношение СОТРУДНИК_
...
принимается за его первичный ключ. Обычно первичным ключом назначается тот возможный ключ, которым проще всего пользоваться при повседневной работе. Остальные возможные ключи, если они есть, называются альтернативными ключами. Для индикации связи между
...
![Рис. 5.2. Концептуальная модель для примера](/5.2.jpg)

_Рис. 5.2. Концептуальная модель для примера_
...
![Рис. 5.3. Реляционная схема базы данных для примера](/5.3.jpg)

_Рис. 5.3. Реляционная схема базы данных для примера_
...
- ЧИТАЕТ — бинарное отношение с составным первичным ключом **Код\_лек**, Код_предм, с кардинальным числом, равным шести, в котором присутствуют первичные ключи только читающих лекторов и первичные ключи только читаемых предметов.

![Рис. 5.4. Реляционные отношения модели для примера](/5.4.jpg)

_Рис. 5.4. Реляционные отношения модели для примера_
...
- Позволяется удалять записи в родительском отношении независимо от существования связанных записей в дочернем отношении.
...
|Ном\_зач\_кн| ФИО\_студента| Код\_группы | ФИО\_старосты |Куратор|
|---|----|---|---|---|
...
В данном отношении с первичным ключом **Ном\_зач\_кн** в каждом
...
всем атрибутам описания студента, в том числе и атрибуту **Ном\_зач\_кн**,
...
|Ном\_зач\_кн| ФИО_студента| Код_группы|
|---|----|---|
...
в каждом кортеже значение атрибута **ФИО\_старосты** заменить Рябов В.С.
на Сенова А.Л.
...
старосты группы требует изменения значения атрибута **ФИО\_старосты**
...
_Определение 1_. Атрибут называется посторонним для
...
_Определение 2._ Функциональная зависимость Х → Y называется
...
_Определение 3._ Для данной схемы отношения R атрибут А в R и
...
Рассмотрим отношение **КОНСУЛЬТАЦИИ\_ДИПЛОМНИКОВ** со
схемой:

**(Таб\_Ном\_преп, Ном\_зач\_кн, Дата, ФИО\_преп, Должность,
ФИО\_студ, Тема\_диплома, Время, Аудитория, Вместимость).**
...
**(Таб\_Ном\_преп, Ном_зач_кн, Дата) → (ФИО\_преп, Должность,
ФИО\_студ, Тема\_липлома, Время, Аудитория, Вместимость).**

Описательные атрибуты преподавателя **ФИО\_преп**, Должность
зависят только от части первичного ключа. Данную ситуацию определяет
зависимость:
**Таб-Ном\_преп → (ФИО\_преп, Должность).**  

Описательные атрибуты студента **ФИО\_студ**, **Тема\_диплома** также
зависят только от части первичного ключа и не зависят от остальных
атрибутов ключа, то есть имеется зависимость вида:

**Ном\_зач\_кн → (ФИО\_студ. Тема\_диплома).**
...
**ПРЕПОДАВАТЕЛЬ (Таб\_Ном\_преп, ФИО\_преп, Должность);
СТУДЕНТ (Ном\_зач\_кн, ФИО\_студ, Тема\_диплома);
КОНСУЛЬТАЦИИ (Таб\_Ном\_преп, Ном\_зач\_кн, Дата, Время,
...
**(Таб\_Ном\_преп, Ном\_зач\_кн, Дата) → Аудитория → Вместимость.**
...
**ПОСТАВКА (Индекс\_поставщ, Имя\_поставщ, Индекс\_товара,
Колич\_товара).**

Допустим также, что значения атрибута **Имя\_поставщ** уникальны и
могут быть использованы наряду с атрибутом **Индекс\_поставщ** для
идентификации поставщика. В такой ситуации можно выделить два
составных потенциальных ключа:

**(Индекс поставщ, Индекс\_товара);
(Имя\_поставщ, Индекс\_товара).**

В рассматриваемом отношении есть два атрибута **Индекс\_поставщ**
и **Имя\_поставщ**, которые идентифицируют один и тот же экземпляр сущности, а, значит, они определяют друг друга. В таком случае отношение содержит два детерминанта, но эти детерминанты не являются
...
детерминанта **Индекс\_поставщ** и **Имя\_поставщ** определяют друг друга,
то возможны два равносильных варианта декомпозиции, приводящей к
НФБК.

Первый вариант получается, если учитывается зависимость

**Индекс\_поставщ → Имя\_поставщ,**

в результате чего имеем следующих два отношения:

**ПОСТАВКА (Иидекс\_поставщ, Индекс\_товара, Колич\_товара);
ПОСТАВЩИК (Индекс\_поставщ, Имя\_поставщ),**

Второй вариант исходит из зависимости

**Имя\_поставщ → Индекс\_поставщ,**

в результате чего получаем альтернативную группу отношений;

**ПОСТАВКА (Имя\_поставщ, Индекс\_товара, Колич\_товара);
ПОСТАВЩИК (Индекс\_поставщ, Имя\_поставщ).**
...
Рассмотрим следующую схему отношения: **НИР (Номер\_НИР,
Сотр, Задан\_НИР).**
...
В том и только в том случае, если множество значений В,
...
- проверка поддержки целостности данных. Рассмотрим последовательно каждое действие.
...
![Рис. 6.1. Диаграмма бинарной связи](/6.1.jpg)

_Рис. 6.1. Диаграмма бинарной связи_
...
**Ф\_И\_О** — фамилия, имя, отчество преподавателя;

**Тел\_П** — телефон преподавателя;

**НК** — номер курса;

**Назв\_К** — название курса.

![Рис. 6.2. Диаграмма бинарной связи типа 1 : 1](/6.2.jpg)

_Рис. 6.2. Диаграмма бинарной связи типа 1 : 1_

Считая, что класс принадлежности является обязательным для обеих
сущностей, получаем отношение:

**ПРЕПОДАВАТЕЛЬ (ТН, Ф\_И\_О, Тел\_П, НК, Назв\_К).**
...
**ПРЕПОДАВАТЕЛЬ (ТН, Ф\_И\_О, Тел_П, НК);**
...
**ПРЕПОДАВАТЕЛЬ (ТН, Ф\_И\_O, Тел_П);**
...
![Рис. 6.3. Диаграмма бинарной связи типа 1 : N](/6.3.jpg)

_Рис. 6.3. Диаграмма бинарной связи типа 1 : N_
...
**ПРЕПОДАВАТЕЛЬ (ТН, Ф\_И\_О, Тел\_П);**
...
![Рис. 6.4. Диаграмма связи "суперкласс/подкласс"](/6.4.jpg)

_Рис. 6.4. Диаграмма связи "суперкласс/подкласс"_
...
![Рис. 7.1. Схема доступа к базе данных](/7.1.jpg)

_Рис. 7.1. Схема доступа к базе данных_
...
![Рис. 7.2. Структура страницы данных](/7.2.jpg)

_Рис. 7.2. Структура страницы данных_

![Рис. 7.3. Определение логической последовательности страниц](/7.2.jpg)

_Рис. 7.3. Определение логической последовательности страниц_

_Слоты_ характеризуют размещение строк данных на странице.
...
![Рис. 7.4. Структура идентификационного номера записи](/7.4.jpg)

_Рис. 7.4. Структура идентификационного номера записи_
...
![Рис. 7.5. Индексированный и индексный файлы](/7.5.jpg)

_Рис. 7.5. Индексированный и индексный файлы_
...
информации так, чтобы не нарушать упорядоченности. Поэтому вся
индексная область файла разбивается на блоки и при начальном
заполнении в каждом блоке остается свободная область (процент
расширения) (рис. 7.6).

![Рис. 7.6. Организация индексно-прямой адресации](/7.6.jpg)

_Рис. 7.6. Организация индексно-прямой адресации_
...
получается _неплотным_ или _разреженным_.
...
![Рис. 7.7. Организация индексно-последовательной адресации](/7.7.jpg)

_Рис. 7.7. Организация индексно-последовательной адресации_
...
![Рис.7.9. Уровни инвертированного списка](/7.9.jpg)

_Рис.7.9. Уровни инвертированного списка_
...
|Код\_дисц |Назв\_дисц |Дата |Оценка|
|---|---|---|---|
...
|Ном\_зач\_кн| ФИО| Код\_дисц |Назв\_дисц |Дата |Оценка|
|---|---|---|----|---|---|
...
|Ном\_зач\_кн |Фамилия |Назв_дисц |Дата |
|---|---|---|---|
...
- проецирование результата предыдущей операции на атрибут **Назв\_дет.** Окончательный результат запроса будет выглядеть следующим образом:
...
{ t.Назв\_дет ⎟ t in r and t.Bec = 2},
...
t.Назв\_дет — значение атрибута **Назв\_дет** в строке r; символ (|) —
разделяет целевой список и определяющее выражение. В данном случае:

t.Назв\_дет — целевой список;
...
В предыдущем примере **Назв\_дет** берется из строки и помешается в
...
t.Hазв\_дет в этой строке не помещается в отношение решения. Затем
...
{t.Код\_дет, t.Назв\_дет, t.Вес ⎟ t in r and t.Bec = 2}.
...
Согласно алгебраическому выражению πНазв\_дет(r ∩ s) реляционной
...
t in r and у in s and t.Кoд\_дет = у.Код\_дет and t.Bec = у.Вес and
t.Hазв\_дет = у.Назв\_дет — определяющее выражение.
...
Такое выражение читается: "Существует строка s в отношении **СКЛАД** такая, что s.Код\_дет = t.Код\_дет и s. Код\_дет > t.Количество".

Приведенное выражение определяет строку t. Если оно истинно, то
есть для строки t существует такая строка s, то t.Hазв\_дет помещается в
результирующее отношение. Если выражение ложно — то есть такой
строки s не существует — тогда t.Назв\_дет не помещается в
результирующее отношение.

Полное решение в реляционном исчислении выглядит следующим
образом:

{t.Назв\_дет ⎟ t in ПОТРЕБНОСТИ and exists s in СКЛАД (s.Код\_дет =
t.Код\_дет and s.Кол\_дет > t.Количество )}.
...
имеет **Назв\_дет = А**, и оно будет помещено в результирующее отношение,
если в отношении **СКЛАД** существует строка, в которой **Код\_дет** = ‘01’, а
Кол\_дет > t.Количество. Такая строка действительно существует, и она
обозначена как s. Итак, t удовлетворяет определяющему условию, поэтому
t.Hазв\_дет помещается в отношение решения. Этот процесс должен
...
|Ном\_зач\_кн| Фамилия| Назв\_дисц |Дата |
|---|---|--|--|
|02-Э-01| Иванов |Физика |10.01.03|
|02-Э-01 |Иванов |Химия |14.01.03 |
|02-Э-02 |Сидоров |Физика |10-01.03|
...
FORALL s {t. Назв\_дисц\_на = s.Назв\_дисц and t.Дата = s.Дата)},
...
отношения **RASP** должна существовать строка t в отношении **VEDOM**, удовлетворяющая условию, то есть в данном случае для каждой строки s отношения **RASP** должна существовать строка t в отношении **VEDOM** с
...
Name\_kaf и Nom\_telef, поэтому сам запрос должен выглядеть следующим
образом:

```
`SELECT Name\_kaf, Nom\_telef`
...
|Kod\_kaf| Name\_kaf| Nom\_telef| Nom\_Auditoria |Col\_sotr| Zav\_kaf|
|---|---|---|---|---|---|
|002 |UhfГ |23-65-43 |003 |22| Махов|
...
по столбцу Name\_kaf в порядке возрастания.
...
ограничиваются только реализацией простых запросов данных из однойтаблицы. Во многих случаях для получения ответа на запрос необходимо
...
|Часов И.И. |Иностр\_язык |5|
|Иванова Я.С. |Иностр\_язык |4|
|Часов И.И.| Экономика |4|
|Иванова Я.С.| Экономика |4|
...
участвовали в выполнении Этапа\_3.
...
по истории, проверяется истинность предиката `NOT EXISTS`. _ИСТИННОСТЬ_ его
...
содержать ключевые слова NOT NULL.
...
![Рис. 10.2. Варианты транзакции](/10.2.jpg)

_Рис. 10.2. Варианты транзакции_
...
**_Проблема потери результатов обновления_**
...
_Рис.10.3. Потеря в момент времени t4 результатов обновления,
выполненного транзакцией А_
...
_Рис.10.4. Транзакция А становится зависимой от невыполненного
изменения в момент времени t2_
...
_Рис.10.5. Транзакция А обновляет невыполненное изменение в момент
...
_Рис.10.6. Транзакция А выполнила несовместимый анализ_
...
_Рис.10.7. Хотя обновления не утрачиваются, но в момент времени t4 возникает тупиковая ситуация_
...
_Рис.10.8. Транзакция А предохраняется от выполнения операций с
незафиксированным изменением в момент времени t2_
...
_Рис.10.9. Транзакция А предохраняется от выполнения операций с
незафиксированным изменением в момент времени t2_
...
| |t5 |Извлечение кортежа Счет 1 (задание X-блокировки для кортежа Счет 1) 30  → 20|
...
_Рис.10. Проблема несовместимого анализа разрешается, но в момент времени t7 возникает тупиковая ситуация_
